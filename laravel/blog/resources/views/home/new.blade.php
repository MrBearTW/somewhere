...
@section('content')
<article class="blogs">
<h1 class="t_nav"><span>您当前的位置：<a href="{{url('/')}}">首页</a>&nbsp;&gt;&nbsp;<a href="{{url('cate/'.$field->cate_id)}}">{{$field->cate_name}}</a>&nbsp;&gt;&nbsp;<a href="/news/s/">日记</a></span><a href="{{url('/')}}" class="n1">网站首页</a><a href="/" class="n2">日记</a></h1>
  <div class="index_about">
    <h2 class="c_titile">{{$field->art_title}}</h2>
    <p class="box_c"><span class="d_time">發布時間：{{data('Y-m-d',$field->art_time)}}</span><span>編輯：{{$field->art_author}}</span><span>查看次数：{{$field->art_view}}</span></p>
    <ul class="infos">
