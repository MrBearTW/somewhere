...
function generateIndexFile(answer) {
  const {
    platform,
    session,
    server
  ...
  const dependenciesLine = `const { ${dependencies.join(', ')} } = require('bottender');`;
  const moduleDepsGroup = [dependenciesLine, requireServerLine(server)].filter(s => !!s).join('\n');
  const peerDepsGroup = requireBotJSONLine(platform);
  const newBotGroup = newBotLines(dependencies[0], platform, sessionStore);
  const handlerGroup = ['bot.onEvent(async context => {', "  await context.sendText('Hello World');", '});'].join('\n');
  const runBotGroup = runBotLines(platform, server);
  return `${[moduleDepsGroup, peerDepsGroup, newBotGroup, handlerGroup, runBotGroup].filter(s => !!s).join('\n\n')}\n`;
}