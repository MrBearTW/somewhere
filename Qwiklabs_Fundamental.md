...
![](https://cdn.qwiklabs.com/NYo2MRjXDjHGhUiLEoQL9MG8gLMn%2B3wrhz53emKj06g%3D)
...
#### Deploy new release
修改三個文件的兩個部分  
更新cluster`skaffold run`  
`kubectl get pods`  
#### Send some data
Kubernetes Engine > Services. Look for the frontend-external
...
#### Create a new table
看一下有哪一些公開的名單`bq ls bigquery-public-data:`  
建立資料庫`bq mk babynames`  
看自己建立的資料庫`bq ls`  
#### Upload the table
取得資料`wget http://www.ssa.gov/OACT/babynames/names.zip`  
`bq load babynames.names2010 yob2010.txt name:string,gender:string,count:integer`  
解壓縮`unzip names.zip`  
建立table(若非UTF8需多加tag)`bq load babynames.names2010 yob2010.txt name:string,gender:string,count:integer`  
`bq ls babynames`  
`bq show babynames.names2010`  
#### Run queries
找出被命名最多的前五名女生`bq query "SELECT name,count FROM babynames.names2010 WHERE gender = 'F' ORDER BY count DESC LIMIT 5"`  
找出被命名最少次的男生名子，至少五次`bq query "SELECT name,count FROM babynames.names2010 WHERE gender = 'M' ORDER BY count ASC LIMIT 5"`  
